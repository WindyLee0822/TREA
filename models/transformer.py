...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input ):#encoder_state, incr_state=None):
        ...
        tensor = input #self.embeddings(input)
        if self.embeddings_scale:
            tensor = tensor * np.sqrt(self.dim)
        tensor = tensor + self.position_embeddings(positions).expand_as(tensor)
...
class TransformerDecoderLayerKG(nn.Module):
    def __init__(
        ...
        self.ffn = TransformerFFN(embedding_size, ffn_size, relu_dropout=relu_dropout)
        self.norm3 = nn.LayerNorm(embedding_size)
        self.special_fuse=nn.Linear(embedding_size*2,embedding_size)
        self.special_gate=nn.Linear(embedding_size,1)

    # def forward(self,x,encoder_output, encoder_mask, kg_encoder_output, kg_encoder_mask, db_encoder_output, db_encoder_mask):#(self, x, x_mask,encoder_output, encoder_mask,word_logit,kg_encoder_output,kg_encoder_mask):
    #     # decoder_mask = self._create_selfattn_mask(x)
    #     # # first self attn
    #     # # n_kind=special_kind_emb.shape[1]
    #     # # bsz=special_kind_emb.shape[0]
    #     # # special_emb=torch.zeros_like(x,device='cuda')
    #     # # for i in range(bsz):
    #     # #     special_emb[i]=special_kind_emb[i][special_mask[i]]
    #     #
    #     # # fuse=self.special_fuse(torch.cat([x,special_emb],-1))
    #     # # # # fuse_gate=torch.sigmoid(self.special_gate(fuse))
    #     # # # # new_x=fuse_gate*x+(1-fuse_gate)*special_emb
    #     # # x=torch.where(special_mask.unsqueeze(-1).repeat(1,1,self.dim)==0,x,fuse+x)
    #     #
    #     # residual = x
    #     # # don't peak into the future!
    #     # x = self.self_attention(query=x, mask=decoder_mask)
    ...
    #     # #     key=word_logit,
    #     # #     value=word_logit,
    #     # #     mask=torch.ones(x.shape[0],6).cuda()
    #     # # )
    #     # # x = self.dropout(x)  # --dropout
    #     # # x = residual + x
...
class TransformerDecoderKG(nn.Module):
    ...
    def forward(self,input, encoder_states, encoder_kg_state, encoder_db_state, incr_state=None):#(self,input,encoder_states,word_logit,encoder_kg_state):
        ...
        # tensor = tensor + self.position_embeddings(positions).expand_as(tensor)
        # tensor = self.dropout(tensor)  # --dropout
        # # word_mask=torch.ones(word_logit.shape[0],word_logit.shape[1]).cuda()
        # kg_encoder_output, kg_encoder_mask = encoder_kg_state
        # for layer in self.layers:
        #     tensor = layer(tensor,input!=0,encoder_output,encoder_mask,word_logit,kg_encoder_output,kg_encoder_mask)
