...
from dataset_publish import dataset,CRSdataset,gen_CRSdataset
...
class TrainLoop_fusion_rec():
    def __init__(self, opt, is_finetune):
        self.opt=opt
        self.train_dataset=dataset('data',opt,mode='train',process_raw_data=False)
        self.val_dataset =dataset('data', self.opt,mode='test',process_raw_data=False)
        self.device='cuda'
    ...
    def build_model(self,is_finetune):
        self.model = CrossModel(self.opt, self.dict,self.con2word,self.n_kind_set, is_finetune).to(self.device)
    ...
    def train(self):
        ...
        for i in range(10):
            ...
            for context, cur_context,response, entity_vec, entity_vector, movie, \
                ...
                num+=1
                # if all_num % 20 == 0:
                #     cur_metric_rec = self.val()
                #     wandb.log({'TREA w/o Aln. Recall@10':cur_metric_rec['recall@10'],'TREA w/o Aln. Recall@50':cur_metric_rec['recall@50']})
            output_metrics_rec = self.val()

            if best_val_rec > output_metrics_rec["recall@50"]+output_metrics_rec["recall@10"]:
                rec_stop=True
            else:
                best_val_rec = output_metrics_rec["recall@50"]+output_metrics_rec["recall@10"]
    ...
    # def metrics_cal_ent(self,rec_loss,scores,ent_labels):
    #     self.metrics_rec_ent["loss"] += rec_loss
    ...
    #             if ent == self.entity_max or ent in self.movie_ids:
    #                 continue
    #             target_idx = ent
    ...
    def metrics_cal_ent(self,rec_loss,scores,ent_labels):
        ...
        for b in range(batch_size):
            for ent in ent_labels[b]:
                if ent == self.entity_max or ent < 6924:
                    continue
                target_idx = ent
                self.metrics_rec_ent["recall@1"] += int(target_idx in pred_idx[b][:1].tolist())
                self.metrics_rec_ent["recall@10"] += int(target_idx in pred_idx[b][:10].tolist())
    ...
    def val(self,is_test=False):
        ...
        # val_dataset=dataset('data', self.opt,mode='test')
        val_set=CRSdataset(self.val_dataset.data_process(),self.opt['n_entity'],self.opt['n_concept'],self.dict)
...
if __name__ == '__main__':
    ...
    if args.is_finetune == 'mov':
        loop=TrainLoop_fusion_rec(vars(args),is_finetune='mov')
        #loop.model.load_model()
        loop.train()
    elif args.is_finetune == 'gen':
