...
EDGE_TYPES = [58, 172]
def _edge_list(graph,is_tgredial):
    if is_tgredial:
        edge_index = [[], []]
        edge_type = []
        for relation in graph['edge']:
            edge_index[0].append(relation[0])
            edge_index[1].append(relation[1])
            edge_type.append(relation[2])
        edge_index = torch.from_numpy(np.array(edge_index)).to(device)
        edge_type = torch.from_numpy(np.array(edge_type)).long().to(device)
        return edge_index, edge_type, graph['n_relation']
...
def concept_edge_list4GCN(is_tgredial):
    if is_tgredial:
        word_kg = json.load(open('tgredial_word_kg.json'))
        edges = word_kg['edge']
...
class CrossModel(nn.Module):
    def __init__(self, opt, dictionary,con2word,n_kind_set, is_finetune='mov', padding_idx=0, start_idx=1, end_idx=2, longest_label=1,is_tgredial=False):
        ...
        self.embeddings = _create_embeddings(
            dictionary, opt['embedding_size'], self.pad_idx,is_tgredial
        )
        ...
        if is_tgredial:
            self.kg=json.load(open('tgredial_kg.json'))
        else:
        ...
        self.db_edge_idx,self.db_edge_type,self.n_relation=_edge_list(self.kg,is_tgredial)
        ...
        self.concept_edge_sets=concept_edge_list4GCN(is_tgredial)
        ...
        if is_tgredial==False:
            w2i=json.load(open('word2index_redial.json',encoding='utf-8'))
        else:
            w2i = json.load(open('word2index_tgredial.json'))
        ...
        if is_finetune!='mov' and is_finetune!='ent':
            params = [self.dbpedia_RGCN.parameters(), self.concept_GCN.parameters(),self.con_classify.parameters(),
    ...
    def forward(self, xs,xs_cur, ys, concept_mask, cur_concept_mask,seed_sets,entity,con_label,db_label, ent_rec,paths,m_paths,cur_l,leaf,test=True, cand_params=None, prev_enc=None, maxlen=None,
                bsz=None):
        """
